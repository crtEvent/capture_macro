...
page = 30  # 찍을 페이지 수
index = 0  # 이미지 파일명 인덱스
picture_size = [417, 144, 865, 715]  # 왼쪽 상단 좌표 , 오른쪽 하단 좌표 (x1, y1, x2, y2)


def replace_start_point(x, y, button, pressed):
    if pressed and button == mouse.Button.left:
        print('입력받은 좌표 : ', (x, y))
        picture_size[0] = int(x)
        picture_size[1] = int(y)
        print(picture_size)
    return False
...
def replace_end_point(x, y, button, pressed):
    if pressed and button == mouse.Button.left:
        print('입력받은 좌표 : ', (x, y))
        picture_size[2] = int(x)
        picture_size[3] = int(y)
        print(picture_size)
    return False
...
class MyApp(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle('스크린샷 매크로 for Mac')
    ...
    def update_mouse_position(self):
        cursor_pos = QCursor.pos() # 마우스 커서 위치 얻기
        ...
        for i in range(page):
            if len(picture_size) >= 4:
                img = pyautogui.screenshot(region=(
                        int(picture_size[0]),
                        int(picture_size[1]),
                        int(picture_size[2] - picture_size[0]),
                        int(picture_size[3] - picture_size[1])
                    )
                )
